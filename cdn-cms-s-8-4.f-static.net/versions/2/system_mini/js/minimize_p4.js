...
function CalendarWidgetV2(settings) {
        ...
        if (!that.calendarSettings.isServiceUnavailable && that.calendarSettings) {
            ...
            if (that.hoursInputSettings) {
                ...
                that.hoursInputOriginalSettings = {
                    ...
                    onInit: function() {
                        if (that.onInit) that.onInit.call(this, that);
                    }
                };
    ...
    function initializeQuickAvailableDays(hoursInputOriginalSettings, selectedDateInCalendarByUser) {
        ...
        if (that.originalOnHoursLoad) that.originalOnHoursLoad.call(this, that, that.HoursInput.$tableHour);
        if (that.onInit) that.onInit.call(this, that);
        ...
        function findAvailableDays(index, daysToShow, baseDate) {
            ...
            hoursInputOriginalSettings.hoursInputSettings.onHoursLoad = function(chWidget, $tableHour) {
                ...
                if (daysToShow.length >= maxButtonsToShow) return;
                if (dateToCheck.isAfter(maxDateToCheck)) return;
...
<div class="quick-day-btn-name">${day.dayName}</div>
    ...
    that.datePickerSubmit = function(calendar, modal) {
        calendar.off('changeDate').on('changeDate', function(ev) {
            ...
            if (that.hoursInputSettings) {
                ...
                that.HoursInput.init({ ...that.hoursInputOriginalSettings,
                    isQuickDaysValidator: false
                });
            }
            if (that.hoursInputSettings) {
    ...
    function checkAndDisableDaysWithNoAvailableTimes($calendar, baseDate, maxDateToCheck, calendarFirstDayOfMonth) {
        ...
        const hoursInputSettings = {
            ...
            hoursInputSettings: { ...that.hoursInputSettings,
                onHoursLoad: function(chWidget, $tableHour) {
                    ...
                    if (that.originalOnHoursLoad) that.originalOnHoursLoad.call(this, that, $tableHour);
                }
            },
            $moduleSelector: that.$moduleSelector,
            moduleID: that.moduleID,
        ...
        function checkDay(dateToCheck) {
            ...
            that.HoursInput.init({ ...hoursInputSettings,
                isQuickDaysValidator: true
            });
        }
    }
    that.HoursInput = function() {
        ...
        function refreshBusinessHours() {
            ...
            function ordersLoadedCallback(data) {
                ...
                if (_.onHoursLoad) _.onHoursLoad.call(this, that, _.$tableHour);
                if (_.isPageLoad) {
                    setTimeout(() => {
                        hideHoursLoading();
                    }, 100);
                ...
                function removeOptions(validatorType) {
                    if (Object.keys(timesToRemove).length > 0) {
                        _.$tableHour.find('option').each(function() {
                            let $option = $(this);
                            let timeValue = $option.val();
                            if (!timesToRemove[timeValue]) return;
            ...
            function buildHourSelectBox(unavailableHours) {
                ...
                _.$tableHour.off('change').on('change', function(event) {
                    if (_.onHoursChage) _.onHoursChage.call(this, $(this).val(), $(this).find('option:selected'));
                });
            }
            ...
            function buildSessionsHourSelectBox(sessions) {
                ...
                $.each(sessions, function(index, session) {
                    if (dayOfWeek != dayjs(session.startDateFUN).day()) return;
                    if (dateFromCalendar != dayjs(session.startDateFUN).format('YYYY-MM-DD')) return;
                    ...
                    if (dayjs().format('YYYY-MM-DD') == dayjs(`${session.startDateFUN}`).format('YYYY-MM-DD')) {
                        if (dayjs(`${session.startDateFUN}`).valueOf() < startTime) return;
                    }
