...
const ColorsDetector = function() {
    ...
    _.getBrightnessDiff = function(hex1, hex2) {
        if (hex1.charAt(0) !== '#') hex1 = '#' + hex1;
        if (hex2.charAt(0) !== '#') hex2 = '#' + hex2;
...
S123.galleriesImageResize = function() {
    ...
    _.resize = () => {
        if ($(_.settings.container).length == 0) return;
    ...
    _.getSize = ($this, magnificSize) => {
        ...
        if (magnificSize) width = magnificSize;
    ...
    _.setSize = ($this) => {
        if (!$this.attr(_.settings.attr)) return;
        if ($this.attr('data-img-resize') && $this.attr('data-img-resize') == 'true') return;
        let attribute = '';
    ...
    _.resizePopupImage = ($this) => {
        if (!$this.attr(_.settings.magnificAttr)) return;
        if ($this.attr('data-img-resize') && $this.attr('data-img-resize') == 'true') return;
    ...
    _.numberOfImagesInRow = () => {
        if (_.settings.section.find(_.settings.container).length == 0) return;
        let rowNumber = _.settings.section.find(_.settings.container).data('row-number'); // example 'in-row-4 mobile-in-row-2'
        if (rowNumber) {
...
var isMobileDevice = (function() {
    ...
    return {
        Android: function() {
            checkCacheValidity();
            if (_cache.Android === undefined) _cache.Android = navigator.userAgent.match(/Android/i);
            return _cache.Android;
        },
        BlackBerry: function() {
            checkCacheValidity();
            if (_cache.BlackBerry === undefined) _cache.BlackBerry = navigator.userAgent.match(/BlackBerry/i);
            return _cache.BlackBerry;
        },
        iOS: function() {
            checkCacheValidity();
            if (_cache.iOS === undefined) _cache.iOS = navigator.userAgent.match(/iPhone|iPad|iPod/i);
            return _cache.iOS;
        },
        Opera: function() {
            checkCacheValidity();
            if (_cache.Opera === undefined) _cache.Opera = navigator.userAgent.match(/Opera Mini/i);
            return _cache.Opera;
        },
        Windows: function() {
            checkCacheValidity();
            if (_cache.Windows === undefined) _cache.Windows = navigator.userAgent.match(/IEMobile/i);
...
S123.Jarallax = function() {
    ...
    _.init = function() {
        $(document).on('s123.page.ready', function(event) {
            $('.parallax-window').each(function() {
                var $elem = $(this);
                if ($elem[0] && $elem[0].jarallax) return;
                _.set($elem);
            });
            _.refresh();
    ...
    _.set = function($jarallax) {
        ...
        if (direction === 'preview-disabled') {
            ...
            $jarallax.each(function() {
                ...
                if (instance && instance.onScroll) {
                    instance.onScroll = function() {}; // Empty function = no-op
                }
            });
            $jarallax.each(function(index, jarallaxElem) {
                ...
                var scrollHandler = function() {
                    ...
                    var scrollOffset = (viewportCenter - elementCenter) * (isMobileDevice.any() ? 0.1 : 0.05); // Reduced speed factor
                    var translateX = 0;
                    var translateY = 0;
                    var zoomFactor;
                    if (direction === 'left') {
                        translateX = -(scrollOffset * 0.1); // Negative = move left
                        var scrollDistance = Math.abs(scrollOffset);
                        zoomFactor = initialScale + (scrollDistance * 0.001);
                    } else if (direction === 'right') {
                        translateX = (scrollOffset * 0.1); // Positive = move right
                        var scrollDistance = Math.abs(scrollOffset);
                        zoomFactor = initialScale + (scrollDistance * 0.001);
        ...
        (function applyJarallaxEffects(attempts) {
            var maxAttempts = 30; // ~500ms at 60fps
            var $jarallaxContainer = $jarallax.children('.jarallax-container');
            if ($jarallaxContainer.length > 0) {
    ...
    _.RefreshManager = (function() {
        ...
        var REFRESH_PRIORITY = {
            'full': 1, // Full refresh (destroy + recreate) - highest priority
        ...
        function isElementReady($element) {
            ...
            if ($element.hasClass('parallax-window')) {
                ...
                if (hasJarallaxData && !hasContainer) return false;
            }
            return true;
        }
        ...
        var manager = {
                ...
                if (existing) {
                    ...
                    if (onComplete) {
                        ...
                        existing.onComplete = function(success) {
                            if (oldCallback) oldCallback(success);
                            onComplete(success);
                        };
                    }
                    return;
            ...
            processQueue: function() {
                ...
                if (pendingRefreshes.size === 0) return;
                isProcessing = true;
                requestAnimationFrame(function() {
            ...
            flush: function() {
                pendingRefreshes.forEach(function(item, key) {
                    try {
                        ...
                        if (item.onComplete) item.onComplete(true);
                    } catch (e) {
                        if (item.onComplete) item.onComplete(false);
                    }
                });
...
function GoToTopButtonInitialize() {
    $(document).on('s123.page.ready', function(event) {
        ...
        $(window)
            ...
            .on('scroll.GoToTopButton', function() {
...
function Site123AdButtonInitialize() {
    ...
    $(document).on('s123.page.ready', function(event) {
        ...
        $(window)
            ...
            .on('scroll.Site123AdBanner', function() {
                bannerHandler();
            });
    });
...
function buildPopup(popID, title, content, iframeURL, closeEsc, closeEnter, oneColor, closeLocation, customClasses) {
    ...
    $(document)
        ...
        .on('keyup.buildPopup_' + popID, function(e) {
...
S123.VideoHandler = function() {
    ...
    _.init = function() {
        ...
        $('.s123-video-handler[data-is-full-screen="true"]')
            ...
            .on('click.VideoHandler', function(event) {
                var $this = $(this);
    ...
    _.addClickEvent = function($el) {
        $el
            .off('click.VideoHandler')
            .on('click.VideoHandler', function() {
...
S123.Pjax = function() {
    ...
    that.init = function() {
        ...
        if ($('#onepage').val() !== '0' && $('#onepage').val() !== '3') return;
...
S123.CrossOriginHandler = function() {
    ...
    _.init = function() {
        ...
        $(window)
            ...
            .on('message.crossOriginHandler', function(event) {
            ...
            if (settings.onepage == '2' && (btns > 1 || icons > 3 || (btns + icons) > 3)) {
                ...
                $html.find('a.l-pagebars-btn').off('click').on('click', function(e) {
                    ...
                    $multiCurrencyBtn
                        ...
                        .on('click.multiCurrencyBtn', function(e) {
                            e.preventDefault();
                            e.stopPropagation();
                        });
...
S123.loadCookieConsent = function() {
    ...
    if ($cookieConsentCountries.length == 0 || $cookieConsentCountries.val().length == 0) return;
...
class ModulesMobileCarouselView {
    ...
    init() {
        ...
        if (!$.isNumeric(mobileViewTypeNUM)) return this;
    ...
    categoryChangeHandler() {
        ...
        this.sectionOBJ
            .off(eventNamespace)
            .on(eventNamespace, (event, category) => {
                this.init();
            });
        return this;
    }
